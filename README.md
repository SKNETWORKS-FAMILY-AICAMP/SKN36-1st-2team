...
| 🗺 **지도 기반 검색·시각화** | 249개 시군구 경계 지도에서 검색·선택·비교 | RF-1.3 |
...
**정규화** — 각 지표는 249개 시군구 기준 **백분위(0~100)** 로 환산합니다. min-max 방식은 이상치 하나에 크게 흔들리기 때문입니다.
...
| `pages/문의.py` | FAQ 조회 및 문의 입력·등록 처리 |
...
| `/recommend` | **유망지역 추천** | 중요도 진단 4문항 → 가중치 반영 순위 → 항목별 근거 |
| `/문의` | **FAQ · 문의** | FAQ 조회, 문의 작성 및 관리자 답변 |
...
`database/migrations/` 스크립트는 **신규 DB 초기화용이 아닙니다.** 이미 만들어 둔 기존 Docker 볼륨을 최신 스키마에 맞출 때만 수동으로 실행합니다. 신규 환경은 `01_schema.sql` + `02_seed.sql` 만으로 최신 상태가 됩니다.
